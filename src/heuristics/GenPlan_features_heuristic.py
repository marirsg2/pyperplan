...
def SOCKET_compute_NN_wGPF_heuristic(state_frozen_set_proposition_strings,goal_frozen_set_proposition_strings):
    global clientsocket
    cwd = os.getcwd()
    ...
    clientsocket.send(bytes(problem_file_desc))
    print('  Problem sent')
...
class GPfeaturesHeuristic(Heuristic): #class name must end with "Heuristic"
    ...
    def __call__(self, node):
        ...
        h = SOCKET_compute_NN_wGPF_heuristic(node.state,self.task.goals)
